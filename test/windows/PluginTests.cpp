...
class PluginTests
{
    ...
    WSL2_TEST_METHOD(HostCrashIsolation)
    {
        ...
        StartWsl(0);
        const DWORD pidBefore = GetWslServicePid();
        ...
        const DWORD pidAfter = GetWslServicePid();
    ...
    {
        // The hook spawns 4 threads behind a start-gate; each thread calls
        // MountFolder + ExecuteBinary in parallel. Per-thread logging is
        ...
        constexpr auto ExpectedOutput =
            ...
            VM created (settings->CustomConfigurationFlags=0)
            Concurrent callbacks complete: success=4 failures=0
            Distribution started, name=test_distro, package=, PidNs=*, InitPid=*, Flavor=debian, Version=13
            Distribution Stopping, name=test_distro, package=, PidNs=*, Flavor=debian, Version=13
            VM Stopping)";
    ...
    {
        // Happy-path drain test: 4 detached workers loop ExecuteBinaryInDistribution
        // (with /bin/true, sub-ms callback) while the distro is alive. They keep
        // calling across OnDistroStopping and _VmTerminate; the exclusive
        // m_callbackLock acquire in _VmTerminate must drain in-flight callbacks
        // before resetting m_utilityVm. Workers exit on the first failure observed
        // AFTER OnVmStopping has set the stop signal — which is guaranteed once
        // m_utilityVm.reset() makes subsequent calls fail (post-drain).
        //
        // Scope:
        ...
        // Give detached workers a moment to observe the post-drain failure
        // (m_utilityVm reset) and exit. The stop signal is set in
        // OnVmStopping, so workers exit on their next failed call.
        Sleep(500);

        // Subsequent WSL command must still succeed — service survived.
        StartWsl(0);
